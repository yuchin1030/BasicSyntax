	...
	UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	if (player != nullptr)
	{
		...
		{
			// 자신의 Physics 옵션을 꺼준다.
			boxComp->SetSimulatePhysics(false);		// physics 를 먼저 꺼주고 attach 해줘야 attach 가 정상 작동됨.
			...
			// 부착될 당시의 플레이어와 자기 자신의 world transform 값을 relative transform 값에 적용시킨다.
			...
			// 플레이어의 애니메이션 인스턴스의 useSniper 변수 값을 변경한다.
