...
void AMarioRealRecentCharacter::Throw(const FInputActionValue& Value)
{
	...
	if (isOnMario)
	{
		...
		// 마리오 던지는 모자 생성
		spawnedCap = GetWorld()->SpawnActor<AMarioThrowingCap>(throwCaps, spawnLoc, spawnRot, params);
		...
		Info.ExecutionFunction = "infoFunc";

		UKismetSystemLibrary::Delay(GetWorld(), 0.5f, Info);*/

		// delay (C++)
		FTimerHandle returnHadler;
		AMarioThrowingCap* mtc = spawnedCap;

		GetWorldTimerManager().SetTimer(returnHadler, mtc, &AMarioThrowingCap::ReturnToPlayer, 0.5f, false);	// false : 1번만, true : 루프
...
/*void AMarioRealRecentCharacter::infoFunc()
{
	spawnedCap->ReturnToPlayer();
}*/
